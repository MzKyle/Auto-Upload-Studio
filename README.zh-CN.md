...
OSS、腾讯云 TurboS3，或同时上传到两个云端。应用持续监控单次焊接目录，文件稳定后
...
有效 `YYYY-MM-DD` 日期目录中的每个直接子目录会成为持续同步任务。目录内部新增或
修改的文件稳定后会上传到同一相对路径；日期目录根部的普通文件不会上传。
...
系统日期跨天且所有已发现的单次焊接目录都已同步或明确跳过后，会在日期目录写入
