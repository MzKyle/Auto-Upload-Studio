# 任务队列与上传执行

## 队列与并发

`TaskQueueService` 每 2 秒检查 `pending` 任务，并同时判断时间窗口和任务并发。

| 配置 | 默认值 | 作用 |
| --- | --- | --- |
| `maxConcurrentTasks` | `5` | 同时运行的逻辑任务 |
| `maxFilesPerTask` | `6` | 单任务内文件作业数 |
| `maxConcurrentUploads` | `30` | 所有任务和云端共享的上传信号量 |

双云文件会产生两个提供方作业，但仍受全局上传信号量限制。

## 执行流程

1. 读取任务创建时锁定的目标模式和 Prefix。
2. 校验所有目标提供方配置并创建任务级 uploader。
3. 扫描并过滤文件，注册逻辑文件和分云文件目标。
4. 读取每个目标的 `pending / uploading / failed` 文件。
5. 为对应提供方构建 `{prefix}/{uploadRelativePath}/{relativePath}`。
6. 并发上传并分别更新云端进度、文件状态和错误。
7. 聚合逻辑文件、逻辑任务和日期汇总状态。
8. 持续写入包含逐云信息的 `process_task.json`。

## 恢复与重试

- 已完成的分云文件目标不会重传。
- 应用崩溃或暂停留下的 `uploading` 会作为未完成状态恢复。
- 瞬时错误最多额外重试 2 次，使用指数退避。
- 指定 `provider` 重试时只重置该云端；未指定时按兼容逻辑处理任务失败目标。

可重试错误包括 `429`、`5xx`、连接重置、超时、DNS 临时失败和管道中断。

## 过滤与标记文件

过滤顺序：

```text
白名单 > 黑名单 > 正则排除 > 后缀匹配
```

隐藏目录以及 `tmp_upload.json`、`process_task.json`、`day_upload.json` 不参与上传。

## 暂停和取消

运行中 uploader 使用独立取消控制。暂停会中止当前任务并保留未完成目标以便恢复；
取消会将任务标记为失败。一个任务的取消不会主动中止其他任务的 client。
